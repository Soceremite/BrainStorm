...
<rapid:override name="breadcrumb">
    ...
    <nav class="breadcrumb">
        ...
        搜索 ${thoughtSearchVoList.get(0).query} 找到 ${thoughtSearchVoList.get(0).page.totalCount} 个与之相关的想法
    </nav>
    <%--面包屑导航 end--%>
</rapid:override>
            ...
            <main id="main" class="site-main" role="main">
                <ul class="search-page">
                    <c:choose>
                        ...
                        <c:otherwise>
                            <br>
                            很遗憾，没有查询到带有 <font style="color: red;"> ${thoughtSearchVoList[0].query} </font> 的内容，换一个关键词再试试吧。
                            <br> <br>
                        </c:otherwise>
                    </c:choose>
                </ul>
            </main>
            <c:if test="${thoughtSearchVoList[0].page.totalCount!=0}">
                <%--分页 start--%>
                <nav class="navigation pagination" role="navigation">
                    <div class="nav-links">
                        ...
                        <c:choose>
                            ...
                            <c:otherwise>
                                <a class="page-numbers"
                                   href="/p/${thoughtSearchVoList[0].page.pageNow-1}/search?query=${thoughtSearchVoList[0].query}">
                        ...
                        <c:if test="${begin >= 2 }">
                            <a class="page-numbers"
                               href="/p/1/search?query=${thoughtSearchVoList[0].query}">1</a>
                        </c:if>
                        <%--显示点点点--%>
                        <c:if test="${begin  > 2 }">
                        ...
                        <c:forEach begin="${begin }" end="${end }" var="i">
                            <c:choose>
                                ...
                                <c:otherwise>
                                    <a class="page-numbers"
                                       href="/p/${i}/search?query=${thoughtSearchVoList[0].query}">${i }</a>
                                </c:otherwise>
                            </c:choose>
                        </c:forEach>
                        <%-- 显示点点点 --%>
                        ...
                        <%-- 显示最后一页的数字 --%>
                        <c:if test="${end < thoughtSearchVoList[0].page.totalPageCount }">
                            <a href="/p/${thoughtSearchVoList[0].page.totalPageCount}/search?query=${thoughtSearchVoList[0].query}">
                        ...
                        <c:choose>
                            ...
                            <c:otherwise>
                                <%--如果没查询到结果，隐藏最后一个>--%>
                                <c:if test="${thoughtSearchVoList[0].page.totalPageCount>0}">
                                    <a class="page-numbers"
                                       href="/p/${thoughtSearchVoList[0].page.pageNow+1}/search?query=${thoughtSearchVoList[0].query}">
